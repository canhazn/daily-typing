  ...
  getRandomLikedNote() {
    return this.getUser().pipe(
      ...
      map(path => {        
        ...
        // If random == 1  ==> state = '<';
        let state: firestore.WhereFilterOp = Math.floor(Math.random() * 2) + 1 == 1 ? '<' : '>';        
        return this.afs.collection(path, ref => ref.where("like", "==", like).where("noteId", state, id).limit(1))
      }),
      switchMap(noteCollection => noteCollection.valueChanges().pipe(map(([note]) => note))),
      switchMap(note => !note ? throwError("There is nothing") : of(note)),      
      retry(10),
      filter((note: Note) => !!note.noteId), // In case it throwError
    )
  }
}
